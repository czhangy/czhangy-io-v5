...
A composable controls bar for list/collection pages. Optionally renders a leading group (back-link + dropdown filter + admin-gated add button) and a trailing group pairing the live-filter search input with pagination, arranged in a single responsive row. Any modal opened by the add button is rendered via `children`.
...
| `filter`     | `{ value: string; options: string[]; maxLabel: string; onChange: (value: string) => void }` | No       | —       | Renders a single-select filter dropdown in the left group, prefixed with an "All" option; `value: ''` represents "All" |
...
- `ALL_OPTION` — the sentinel label ("All") shown for the unfiltered state and prepended to `filter.options`

## Handlers

- `handleFilterChange` — translates the selected dropdown label back to `filter.onChange`, mapping `ALL_OPTION` to `''`

## Rendering

- `hasLeading` — whether the leading group renders at all; it's only emitted when at least one of `backLink`/`filter`/`add` is supplied. Grouping these together keeps them as one non-splitting unit, so `justify-content: space-between` only ever separates the leading group from `trailing` as a whole — never individual atoms within it when the row wraps onto two lines
