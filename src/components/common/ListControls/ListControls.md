...
Controls bar for a paginated admin-editable list page. Renders a back link to Status on the left, and an admin-only "add" button plus pagination on the right. Any modal opened by the add button is rendered via `children`.
